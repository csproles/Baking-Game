...
public class MovementController {
    ...
    private PickUpStation pickUpStation = new PickUpStation();
    ...
    File fButtonFile = new File("hproject\\src\\main\\resources\\pressF.png");
    Image fButtonImage = new Image(fButtonFile.toURI().toString());
    ImageView fButtonImageView = new ImageView(fButtonImage);
    ...
    public void makeMoveable(ImageView player, Scene scene){
        ...
        fButtonImageView.setFitWidth(75);
        fButtonImageView.setFitHeight(75);
        fButtonImageView.setPreserveRatio(true);
        fButtonImageView.setTranslateY(-100);
        ...
        public void handle(long timestamp){
            if(wPressed.get()){
                if(player.getLayoutY() > 15){
                    ...
                    Level1.level1Pane.getChildren().remove(fButtonImageView);
                    ...
                    if(player.getLayoutY() < 75 && ((player.getLayoutX() > 62 && player.getLayoutX() < 207.5))){
                        ...
                        if(Constants.HAS_ORDERED && !(Constants.CAKE_MIXED)){
                            backAnimation.stopAnimation();
                            fButtonImageView.setTranslateX(-((Constants.PLAYABLE_PANE_WIDTH/2) - player.getLayoutX() - 15));
                            fButtonImageView.setTranslateY(-115);
                            Level1.level1Pane.getChildren().addAll(fButtonImageView);
                        }
                    }
            ...
            if(sPressed.get()){
                if(player.getLayoutY() < 350){
                    ...
                    if(player.getLayoutY() > 300 && (player.getLayoutX() > -60 && player.getLayoutX() < 130)){
                        ...
                        if(!(Constants.HAS_ORDERED)){
                            backAnimation.stopAnimation();
                            fButtonImageView.setTranslateX(-((Constants.PLAYABLE_PANE_WIDTH/2) - player.getLayoutX()));
                            fButtonImageView.setTranslateY(125);
                            Level1.level1Pane.getChildren().addAll(fButtonImageView);
                        }
                    }
                    //Stopping the player from intersecting with the pick up
                    else if(player.getLayoutY() > 300 && (player.getLayoutX() > 1000 && player.getLayoutX() < 1200)){
                        
                        if(!(Constants.HAS_DELIVERED)){
                            backAnimation.stopAnimation();
                            fButtonImageView.setTranslateX(-((Constants.PLAYABLE_PANE_WIDTH/2) - player.getLayoutX()));
                            fButtonImageView.setTranslateY(125);
                            Level1.level1Pane.getChildren().addAll(fButtonImageView);
            ...
            if(aPressed.get()){
                ...
                if(player.getLayoutX() > -20){
                    ...
                    //Stopping the player from intersecting with the order station
                ...
                if(player.getLayoutX() < 1160){
                    ...
                    //Stopping the player from intersecting with the pick up
                    else if(player.getLayoutY() > 310 && (player.getLayoutX() > 1000 && player.getLayoutX() < 1200)){
                        player.setLayoutX(990);
                    }
                    else{
    ...
    private void movementSetup(){
        ...
        scene.setOnKeyReleased(e -> {
            ...
            if(e.getCode() == KeyCode.F){
                //Order Station
                if(player.getLayoutY() > 290 && (player.getLayoutX() > -20 && player.getLayoutX() < 130)){
                    if(!(Constants.HAS_ORDERED) && !(Constants.CAKE_MIXED) && !(Constants.CAKE_BAKED) && !(Constants.CAKE_DECORATED)){
                        orderStation.start(stage);
                    ...
                    if(Constants.HAS_ORDERED && !(Constants.CAKE_MIXED) && !(Constants.CAKE_BAKED) && !(Constants.CAKE_DECORATED && !(Constants.HAS_DELIVERED))){
                        MixingMechanism mixingMechanism = new MixingMechanism();
                        mixingMechanism.start(stage);
                    }
                }
                else if(Constants.HAS_ORDERED && player.getLayoutY() < 75 && (player.getLayoutX() > 460 && player.getLayoutX() < 670)){
                    if(Constants.HAS_ORDERED && Constants.CAKE_MIXED && !(Constants.CAKE_BAKED) && !(Constants.CAKE_DECORATED && !(Constants.HAS_DELIVERED))){
                        bakingMechanism.start(stage);
                    }
                }
                else if(Constants.HAS_ORDERED && player.getLayoutY() < 75 && (player.getLayoutX() > 860 && player.getLayoutX() < 1080)){
                    if(Constants.HAS_ORDERED && Constants.CAKE_MIXED && Constants.CAKE_BAKED && !(Constants.CAKE_DECORATED && !(Constants.HAS_DELIVERED))){
                        decoratingMechanism.start(stage);
                    }
                }
                else if(player.getLayoutY() > 290 && (player.getLayoutX() > 1000 && player.getLayoutX() < 1200)){
                    if(Constants.HAS_ORDERED && Constants.CAKE_MIXED && Constants.CAKE_BAKED && Constants.CAKE_DECORATED && !(Constants.HAS_DELIVERED)){
                        pickUpStation.start(stage);
                    }
                }
                fPressed = false;
                Level1.level1Pane.getChildren().remove(fButtonImageView);
